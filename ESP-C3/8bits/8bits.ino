...
void rodarControle() {
  ...
  if (ligado) {
    ...
    if(contador % 10 == 0){
      ...
      Controle_Saida = (int)(255.0 * Controle_PWM);
      ...
      if (contador % 30 == 0) { // ~300ms
         ...
         else if (correnteMedida < Referencia && correnteMedida > 0.1 && Controle_Saida == 255){erro = 2; google = true;}
         ...
         */
          ...
          else if (correnteMedida == 0 && Controle_Saida > 125) { erro = 1; google = true; }
          else { erro = 0; google = false; digitalWrite(power, HIGH); }
          } else {
          // Modos pulsados: detecta curto por corrente x PWM + fonte desligada
          bool curtoReal = (correnteMedida > (Referencia * 2.0) && Controle_Saida < 75);
          bool semAlimentacao = (tensaoAtual <= 0.1 && Controle_Saida > 150);

          if (curtoReal || semAlimentacao) { erro = 3; google = true; }
          else if (correnteMedida == 0 && Controle_Saida > 125) { erro = 1; google = true; }
          else if (correnteMedida < Referencia && correnteMedida > 0.05 && Controle_Saida == 255) { erro = 2; google = true; }
          else { erro = 0; google = false; digitalWrite(power, HIGH); }
          } 
